...
**Coverage: exhaustive (42 cases, 6 broken)**
...
- **Total: 42** (36 active + 6 broken)

### Broken (input.broken.mical)

6 cases with body=none + termination=dedent produce parser errors
("unexpected token after value"). The header line immediately followed
by a non-indented key-value pair (no blank line separator) is not supported.
